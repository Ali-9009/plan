...
import { Check, AlertCircle, ChevronLeft, Lock } from "lucide-react";
...
export default function Step_1() {
  ...
  return (
    <div>
      ...
      <section className="bg-linear-to-b from-white via-slate-50 to-white px-4 py-12 text-slate-950">
        <div className="mx-auto max-w-6xl">
            ...
            {stories.map((story) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 rounded-2xl px-2 py-3 text-center transition ${
                    isActive
                ...
                >
                  ...
                  <span
                    className={`block text-[10px] md:text-sm font-semibold ${
            ...
            >
              {activeStory.plans.map((plan) => (
                <SwiperSlide key={plan.sku}>
                  <article
                    className={`relative flex h-full flex-col rounded-2xl border bg-white p-6 shadow-xl shadow-slate-200/60 ${
                      plan.badge
                  ...
                  >
                    ...
                    <Link
                      to="/step-2"
                      className="mt-auto inline-block rounded-xl bg-(--primary-color) px-4 py-2 text-md text-center font-semibold text-white transition hover:bg-(--secondary-color)"
                    >
                      Choose this plan
                    </Link>
                  </article>
          ...
          <div className="hidden gap-6 md:grid md:grid-cols-3">
            {activeStory.plans.map((plan) => (
              <article
                key={plan.sku}
                className={`relative flex flex-col rounded-2xl border bg-white p-6 shadow-xl shadow-slate-200/60 transition hover:-translate-y-1 hover:shadow-2xl ${
                  plan.badge
                    ? "border-blue-300 ring-4 ring-blue-100"
                    : "border-slate-200"
                }`}
              >
                {plan.badge && (
                  <div className="absolute -top-3 left-6 rounded-full bg-blue-600 px-4 py-1 text-xs font-bold text-white shadow-lg">
                    {plan.badge}
                  </div>
                )}
                <div className="mb-5 flex items-start justify-between gap-4 pt-2">
